...
class BoldFormatter(private val context: Context) : Formatter {
    ...
    override fun format(inputText: String, inputSelection: TextRange): Pair<String, TextRange> {
        ...
        if (inputSelection.start != inputSelection.end) {
            ...
            selection = TextRange(
                ...
                (inputSelection.end + 4).coerceAtMost(content.length)
            )

        } else {
            val hint = StringResourceUtils.getStringResource(
                R.string.bold_markdown_hint,
                Language.EN,
                context
            )
            content = inputText
