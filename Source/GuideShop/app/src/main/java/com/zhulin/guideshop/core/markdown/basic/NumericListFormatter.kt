...
class NumericListFormatter(private val context: Context) : Formatter {
    ...
    override fun format(inputText: String, inputSelection: TextRange): Pair<String, TextRange> {
        ...
        if (inputSelection.start != inputSelection.end) {
                    ...
                    .map {
                        "${
                            StringResourceUtils.getStringResource(
                                R.string.numeric_list_markdown_symbol,
                                Language.EN,
                                context
                            )
                        } $it\n"
            ...
            val hint = "\n${
                StringResourceUtils.getStringResource(
                    R.string.numeric_list_markdown_hint,
                    Language.EN,
                    context
                )
            }"
            ...
            selection = TextRange(
                (inputSelection.start + 4).coerceAtLeast(0)
